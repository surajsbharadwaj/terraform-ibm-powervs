...
offeringName=$(jq -r '.name' < offering.json)
...
if [ -z ${EXAMPLE+x} ]; then
    ibmcloud catalog offering version validate --vl "$versionLocator" --override-values valid-values.tfvars --timeout 5400 --workspace-name "$offeringName" --workspace-tf-version 1.1
else
    ibmcloud catalog offering version validate --vl "$versionLocator" --override-values valid-values.json --timeout 5400 --workspace-name "$offeringName" --workspace-tf-version 1.1
fi
